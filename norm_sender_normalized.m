% Y. Zhang and D. M. Chandler, \An algorithm for no-reference image quality
% assessment based on log-derivative statistics of natural scenes," Image
% Quality and System Performance, vol. 8653, p. 86530J, 2013.
...
for scale=1:Nsc
    for orien=1:Nor
        ...
        o_c = (o_c(:));
        o_c = o_c - mean(o_c);
        %[hy, ~] = hist(o_c,nbins);%[hy rangeo] = hist(o_c,nbins);
        %hy=hy/sum(hy);
